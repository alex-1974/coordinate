...
unittest {
  ...
  foreach (record; file.byLine.filter!(a => !a.startsWith("#")).joiner("\n").csvReader!(Tuple!(string, size_t, double, double, double, double))) {
    ...
    writefln("  %.6f %.6f", result.latLo, record[2]);
    writefln("  %.12f %.12f", result.lonLo, record[3]);
    writefln("  %.10f %.10f", result.latHi, record[4]);
    writefln("  %.12f %.12f", result.lonHi.roundTo(10), record[5].roundTo(10));

    assert(result.latLo.roundTo(11) == record[2].roundTo(11)
        && result.latHi.roundTo(11) == record[4].roundTo(11));
    assert(result.lonLo.roundTo(11) == record[3].roundTo(11)
        && result.lonHi.roundTo(10) == record[5].roundTo(10));
